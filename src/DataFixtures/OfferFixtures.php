...
class OfferFixtures extends Fixture implements DependentFixtureInterface
{
    public function load(ObjectManager $manager): void
    {
        ...
        for ($i = 1; $i <= 60; $i++) {
            ...
            for ($j = 0; $j < 6; $j++) {
                $offer->addSkill($this->getReference('skill_soft_' . $faker->unique(true)->numberBetween(0, 7)));
                $offer->addSkill($this->getReference('skill_hard_' . $faker->unique(true)->numberBetween(0, 7)));
            }
